...
export default function DisplayAllCompanyJobs() {
  ...
  const openDialog = (rowData) => {
    ...
    setComapnyName(rowData.companyid);
    setCategoryId(rowData.categoryid);
    setSubCategoryId(rowData.subcategoryid);
    fetchAllSubCategory(rowData.categoryid);
    fetchAllSkills(rowData.subcategoryid);
    setSkills(JSON.parse(rowData.skills));
    fetchAllEducations();
    setEducation(JSON.parse(rowData.educationqualification))
    fetchAllCitys();
    setCity(JSON.parse(rowData.worklocationcity));
    setJobDescription(rowData.jobdescription);
    ...
    setBenefits(rowData.benefits);
    setPostDate(rowData.postdate);
    setDeadlineDate(rowData.applicationdeadline);
    setStartDate(rowData.expectedstart);
    setSupplementalPay(rowData.supplementalpay);
    setcontactPerson(rowData.contactperson);
    SetEmailId(rowData.emailaddress);
    setMobileNo(rowData.mobileno);
    setQuestions(rowData.applicationquestion);
    setOpen(true);
  };
  ...
  const showDialog = () => {
    return (
      <Dialog open={open}>
        <DialogContent>{showCompanyJobsForm()}</DialogContent>
        <DialogActions>
  ...
  const fetchAllCompanyJobs = async () => {
    var response = await getData("companyjobs/display_all_companyjobs");
  ...
  const handleClick = async () => {
      ...
      var response = await postData("companyjobs/edit_companyjobs", body);
  ...
  const handleDelete = async () => {
      ...
      var response = await postData("companyjobs/delete_companyjobs", body);
  ...
  const showCompanyJobsForm = () => {
    return (
      <div>
        <div>
          <Grid container spacing={2}>
            <Grid size={12}>
              <TitleComponent title="Edit Job Registeration" />
            </Grid>
                ...
                return (
                  <li key={option.qualificationid} {...optionProps}>
                    <Checkbox
                      ...
                      style={{ marginRight: 8 }}
                      checked={selected}
                      value={option.qualificationname}
                    />
            ...
            <Grid size={12}>
              ...
              />
              <div className={classes.helperTextStyle}>{formError.city}</div>
            </Grid>
            ...
            <Grid size={6}>
              <FormControl fullWidth>
                ...
                >
                  ...
                  <MenuItem value={"Work From Ofiice"}>
                    Work From Office
                  </MenuItem>
                </Select>
                <div className={classes.helperTextStyle}>
    ...
    return (
      <MaterialTable
        ...
        columns={[
          ...
          //   render: (rowData) => (
          //     <div>
          //       {rowData.educationqualification}
          //     </div>
          //   ),
          //  },
          {
